...
def load_data(file: Union[str, Path], 
              encoding: Optional[str] = None,
              auto_detect_encoding: bool = True,
              date_columns: Optional[List[str]] = None,
              custom_na_values: Optional[List[str]] = None) -> pd.DataFrame:
    """
    ...
    # Default NA values
    na_values = ['', 'NA', 'null', 'NULL', 'NaN', 'nan', 'N/A', '#N/A', '#VALUE!', '#NULL!']
    if custom_na_values:
        ...
        try:
            ...
            if date_columns is None:
                try:
                    ...
                    potential_date_cols = [col for col in peek_df.columns 
    ...
    raise UnicodeDecodeError(f"Could not decode file with any of the attempted encodings: {encodings_to_try}")
    ...
    try:
        ...
        if date_format:
            try:
                ...
                logger.warning(f"Failed to parse dates with format {date_format}, trying automatic parsing")
...
def safe_sort(lst: List[Any]) -> List[Any]:
    """
    Custom sorting function to handle mixed types safely.
    
    This function sorts a list containing mixed data types (strings, numbers, NaN values)
    by converting all values to strings and handling NaN values appropriately.
    ...
    def key_func(item: Any) -> Tuple[int, str]:
        ...
        - First element (0 or 1) determines sort priority (0 for valid values, 1 for NaN)
...
def parse_datetime(date_str: str, 
                  date_formats: Optional[List[str]] = None) -> Optional[datetime]:
    """
...
def validate_csv_structure(df: pd.DataFrame, 
    ...
    if len(df) < min_rows:
        ...
        validation_result['issues'].append(f"DataFrame has {len(df)} rows, minimum required: {min_rows}")
    ...
    if required_columns:
        ...
        if missing_columns:
            validation_result['valid'] = False
            validation_result['issues'].append(f"Missing required columns: {list(missing_columns)}")
    
    # Check for completely empty columns
    empty_columns = df.columns[df.isnull().all()].tolist()
    if empty_columns:
        validation_result['warnings'].append(f"Completely empty columns: {empty_columns}")
    
    # Check for duplicate columns
    duplicate_columns = df.columns[df.columns.duplicated()].tolist()
    if duplicate_columns:
        validation_result['valid'] = False
        validation_result['issues'].append(f"Duplicate column names: {duplicate_columns}")
...
def get_column_summary(df: pd.DataFrame, column: str) -> Dict[str, Any]:
    ...
    if pd.api.types.is_numeric_dtype(col_data):
        ...
        summary.update({
            ...
            'avg_length': col_data.astype(str).str.len().mean() if col_data.count() > 0 else 0
        })
