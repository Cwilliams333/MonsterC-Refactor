...
def analyze_wifi_errors(file, error_threshold: int = 9) -> Tuple[Optional[pd.DataFrame], Optional[go.Figure], Optional[pd.DataFrame], Optional[go.Figure]]:
    """
    ...
    logger.info(f"Successfully analyzed WiFi errors: {len(high_error_operators)} high error operators found")
