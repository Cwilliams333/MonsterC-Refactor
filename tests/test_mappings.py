...
class TestMappingDataIntegrity:
    ...
    def test_device_map_structure(self):
        ...
        for device, code in DEVICE_MAP.items():
            ...
            assert isinstance(
                code, (str, list)
            ), f"Device code for {device} should be string or list"
            if isinstance(code, list):
                assert all(
                    isinstance(c, str) for c in code
                ), f"All codes for {device} should be strings"
    ...
    def test_station_map_structure(self):
        ...
        for station, machine in STATION_TO_MACHINE.items():
            ...
            assert isinstance(
                machine, str
            ), f"Machine name for {station} should be string"
            assert (
                station.lower() == station
            ), f"Station ID {station} should be lowercase"
    ...
    def test_test_fail_map_structure(self):
        ...
        for test, failures in TEST_TO_RESULT_FAIL_MAP.items():
            ...
            ), f"All failures for {test} should be strings"
        ...
        for test, failures in TEST_TO_RESULT_FAIL_MAP.items():
            for failure in failures:
                if failure in seen_failures:
                    pytest.fail(
                        f"Failure '{failure}' appears in both '{test}' and '{seen_failures[failure]}'"
                    )
