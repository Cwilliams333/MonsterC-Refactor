        ...
        for hour in range(24):  # Each hour of the day
            ...
            for _ in range(np.random.randint(1, 8)):  # 1-7 errors per hour
                ...
                if operator in ["STN251_RED(id:10089)", "STN252_RED(id:10090)"]:
                    error_msg = np.random.choice(wifi_errors, p=[0.4, 0.4, 0.2])  # More likely to have WiFi errors
                else:
                    error_msg = np.random.choice(wifi_errors + other_errors, p=[0.1, 0.1, 0.1, 0.2, 0.2, 0.3])
            ...
            for _ in range(np.random.randint(10, 50)):  # 10-50 successful transactions per hour
                operator = np.random.choice(operators)
                data.append({
                    "Date": date_str,
                    "Hour": hour_str,
...
class TestAnalyzeWifiErrors:
    ...
    def test_analyze_wifi_errors_basic(self, sample_wifi_data):
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.csv', delete=False) as tmp_file:
            ...
            try:
                # Test the function
                styled_results, heatmap_fig, styled_pivot, trend_fig = analyze_wifi_errors(file_obj, error_threshold=9)
    ...
    def test_analyze_wifi_errors_high_error_threshold(self, high_error_wifi_data):
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.csv', delete=False) as tmp_file:
            ...
            try:
                ...
                styled_results, heatmap_fig, styled_pivot, trend_fig = analyze_wifi_errors(file_obj, error_threshold=15)
    ...
    def test_analyze_wifi_errors_low_threshold(self, high_error_wifi_data):
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.csv', delete=False) as tmp_file:
            ...
            try:
                ...
                styled_results, heatmap_fig, styled_pivot, trend_fig = analyze_wifi_errors(file_obj, error_threshold=3)
    ...
    def test_analyze_wifi_errors_empty_data(self):
        ...
        empty_df = pd.DataFrame(columns=["Date", "Hour", "Operator", "error_message", "IMEI"])
    ...
    def test_analyze_wifi_errors_different_thresholds(self, sample_wifi_data):
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.csv', delete=False) as tmp_file:
            ...
            try:
                ...
                styled_results_low, _, _, _ = analyze_wifi_errors(file_obj, error_threshold=0)
                
                # Test with very high threshold (99%) - should not find high error operators
                styled_results_high, heatmap_high, pivot_high, trend_high = analyze_wifi_errors(file_obj, error_threshold=99)
    ...
    def test_analyze_wifi_errors_specific_operators(self, sample_wifi_data):
        ...
        target_operators = ["STN251_RED(id:10089)", "STN252_RED(id:10090)", "STN351_GRN(id:10380)", "STN352_GRN(id:10381)"]
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.csv', delete=False) as tmp_file:
            ...
            try:
                styled_results, _, _, _ = analyze_wifi_errors(file_obj, error_threshold=9)
                
                if styled_results is not None:
                    results_df = styled_results.data
                    # Check that only target operators (plus Grand Total) are in results
                    operators_in_results = set(results_df["Operator"].values) - {"Grand Total"}
    ...
    def test_analyze_wifi_errors_specific_error_types(self, sample_wifi_data):
        ...
        target_wifi_errors = ["Device closed the socket", "DUT connection error", "DUT lost WIFI connection"]
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.csv', delete=False) as tmp_file:
            ...
            try:
                styled_results, _, _, _ = analyze_wifi_errors(file_obj, error_threshold=0)  # Low threshold to get results
    ...
    def test_full_workflow_with_high_errors(self, high_error_wifi_data):
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.csv', delete=False) as tmp_file:
            ...
            try:
                ...
                styled_results, heatmap_fig, styled_pivot, trend_fig = analyze_wifi_errors(file_obj, error_threshold=10)
                ...
                assert all(col in results_df.columns for col in ["Operator", "Total Transactions", "WiFi Errors", "Error Percentage"])
