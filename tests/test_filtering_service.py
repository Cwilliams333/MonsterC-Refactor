...
class TestFilteringService:
    ...
    def test_get_unique_values_with_nulls(self):
        ...
        df_with_nulls = pd.DataFrame({"Column": ["A", "B", None, "A", pd.NA, "C"]})
    ...
    def test_analyze_top_errors_by_model(self, sample_test_data):
        ...
        if len(result) > 0:
            assert (
                "iPhone15Pro" in result["Model"].values
                or "iPhone16" in result["Model"].values
            )
    ...
    def test_filter_data_by_operator(self, sample_test_data):
        ...
        result = filter_data(
            sample_test_data, "Filter by Operator", "TestOp1", "All", "All"
        )
    ...
    def test_apply_filter_and_sort_with_sorting(self, sample_test_data):
        ...
        assert (
            models == sorted(models) or len(set(models[:2])) == 1
        )  # Either sorted or first elements are same
