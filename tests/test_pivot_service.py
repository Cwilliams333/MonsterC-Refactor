...
def sample_df():
    ...
    data = {
        ...
        "result_FAIL": np.random.choice(test_cases + [""], n_rows, p=[0.15, 0.15, 0.15, 0.15, 0.4]),
...
class TestCreatePivotTable:
    ...
    def test_create_pivot_table_missing_required_fields(self, sample_df):
        ...
        # Missing rows
        pivot = create_pivot_table(sample_df, rows=[], columns=["Station ID"], values="IMEI")
        assert len(pivot) == 0
        
        # Missing values
        pivot = create_pivot_table(sample_df, rows=["Model"], columns=["Station ID"], values="")
        assert len(pivot) == 0
    ...
    def test_create_pivot_table_multi_level_columns(self, sample_df):
        ...
        assert all(not isinstance(col, tuple) or "_" in str(col) for col in pivot.columns)
...
class TestPivotAnalysisFunctions:
    ...
    def test_find_top_failing_stations(self, pivot_table_df):
        ...
        assert all(top_stations.iloc[i] >= top_stations.iloc[i+1] for i in range(len(top_stations)-1))
    ...
    def test_analyze_top_test_cases(self, pivot_table_df):
        ...
        test_case_data = {
            'result_FAIL': ['6A-Display Fail', '4J-Camera Fail', '2B-Touch Fail', '5K-Audio Fail'],
            'radi135': [10, 8, 5, 3],
            'radi136': [7, 12, 4, 2]
        }
...
class TestEdgeCases:
    ...
    def test_empty_dataframe(self):
        ...
        pivot = generate_pivot_table_filtered(
            empty_df, ["Model"], ["Station ID"], "IMEI", "count",
            "All", "All", "All"
        )
        assert isinstance(pivot, pd.DataFrame)
