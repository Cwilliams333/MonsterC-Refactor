...
def sample_failures_df():
    ...
    for i in range(10):
        data.append({
            "Date": datetime.now() - timedelta(days=i),
            "Model": np.random.choice(["iPhone14ProMax", "iPhone15Pro", "iPhone15", "iPhone14"]),
            "Station ID": np.random.choice(["radi135", "radi136", "radi137", "radi138"]),
            "result_FAIL": "",
            "Overall status": "SUCCESS",
            "IMEI": f"35{5000 + i:013d}",
            "Operator": np.random.choice(["STN251_RED(id:10089)", "STN252_RED(id:10090)"])
        })
...
class TestCreateSummary:
    ...
    def test_create_summary_basic(self, repeated_failures_result_df):
        ...
        assert "| Model | Model Code | Station ID | Test Case | TC Count | IMEI Count |" in summary
    
    def test_create_summary_empty(self):
        """Test creating summary from empty DataFrame."""
        empty_df = pd.DataFrame(columns=["Model", "Station ID", "result_FAIL", "TC Count", "IMEI Count", "Model Code"])
...
class TestAnalyzeRepeatedFailures:
    ...
    def test_analyze_repeated_failures_basic(self, sample_failures_df):
        ...
        summary, fig, interactive_df, dropdown = analyze_repeated_failures(sample_failures_df, min_failures=4)
        ...
        assert "Display Fail" in summary or "Camera Fail" in summary or "Touch Fail" in summary
    ...
    def test_analyze_repeated_failures_min_threshold(self, sample_failures_df):
        ...
        summary, fig, interactive_df, dropdown = analyze_repeated_failures(sample_failures_df, min_failures=5)
        
        # Should find patterns with 5+ failures
        assert isinstance(summary, str)
        
        # Test with min_failures=7 - should find only iPhone15Pro (6 failures won't qualify)
        summary, fig, interactive_df, dropdown = analyze_repeated_failures(sample_failures_df, min_failures=7)
        assert isinstance(summary, str)
    ...
    def test_analyze_repeated_failures_no_failures(self):
        ...
        summary, fig, interactive_df, dropdown = analyze_repeated_failures(success_only_df, min_failures=4)
        assert isinstance(summary, str)
    ...
    def test_analyze_repeated_failures_edge_cases(self):
        ...
        summary, fig, interactive_df, dropdown = analyze_repeated_failures(empty_df, min_failures=4)
        assert isinstance(summary, str)
        
        # DataFrame with missing columns should be handled gracefully
        incomplete_df = pd.DataFrame({"Model": ["iPhone14ProMax"]})
        summary, fig, interactive_df, dropdown = analyze_repeated_failures(incomplete_df, min_failures=4)
        assert isinstance(summary, str)
...
class TestUpdateSummaryChartAndData:
    ...
    def test_update_summary_chart_and_data_sort_by_model(self, repeated_failures_result_df):
    ...
    def test_update_summary_chart_and_data_filter_test_cases(self, repeated_failures_result_df):
        """Test updating with test case filtering."""
        # Test filtering by specific test cases
        selected_cases = ["6A-Display Fail (5) max failures", "4J-Camera Fail (6) max failures"]
        summary, fig, interactive_df = update_summary_chart_and_data(
    ...
    def test_update_summary_chart_and_data_select_all(self, repeated_failures_result_df):
    ...
    def test_update_summary_chart_and_data_empty_input(self):
        ...
        summary, fig, interactive_df = update_summary_chart_and_data(None, "TC Count", [])
...
class TestUpdateSummary:
    ...
    def test_update_summary_filter_cases(self, repeated_failures_result_df):
        ...
        summary = update_summary(repeated_failures_result_df, "TC Count", selected_cases)
        assert isinstance(summary, str)
